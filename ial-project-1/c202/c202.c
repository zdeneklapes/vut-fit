...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	if (!Stack_IsEmpty(stack)) { // if not empty return char
		*dataPtr =  stack->array[stack->topIndex];
	} else { // else error -> stack is empty
...
void Stack_Pop( Stack *stack ) {
	if (!Stack_IsEmpty(stack)) { // top will be pointed to top of the stack
...
void Stack_Push( Stack *stack, char data ) {
	if (!Stack_IsFull(stack)) { // if not full add char
		stack->topIndex++;
        stack->array[stack->topIndex] = data;
	} else { // else error -> stack is full
